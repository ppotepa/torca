...
try {
    ...
    if (-not $SkipFlutter) {
        ...
        try { Invoke-Checked 'Flutter dependencies' { flutter pub get }; Invoke-Checked 'Dart formatting' { dart format --output=none --set-exit-if-changed lib test }; Invoke-Checked 'Flutter analysis' { flutter analyze }; Invoke-Checked 'Flutter tests' { flutter test } } finally { Pop-Location }
    }
