...
class MemoryEngineGateway implements EngineGateway {
  ...
  Future<BridgeResultDto> execute(BridgeCommandDto command) async {
    ...
    if (command is CreatePairingCommandDto || command is JoinPairingCommandDto) {
      ...
      if (current.pairings.any((item) => item.id == id)) return const BridgeResultDto(ok: false, kind: 'error', error: 'pairing already exists');
      final pairing = PairingDto(
        id: id, code: code, role: joining ? 'joiner' : 'creator', state: 'open',
        expiresAtMs: DateTime.now().millisecondsSinceEpoch + const Duration(minutes: 5).inMilliseconds,
        localApproved: false, remoteApproved: false,
      );
    ...
    if (command is ApprovePairingCommandDto) {
      return _updatePairing(current, command.sessionIdHex, (pairing) => PairingDto(
        id: pairing.id, code: pairing.code, role: pairing.role, state: 'approved',
        expiresAtMs: pairing.expiresAtMs, localApproved: true, remoteApproved: pairing.remoteApproved,
      ), 'pairing_updated');
    }
    if (command is RejectPairingCommandDto) return _terminalPairing(current, command.sessionIdHex, 'rejected', 'pairing_rejected');
    if (command is CancelPairingCommandDto) return _terminalPairing(current, command.sessionIdHex, 'cancelled', 'pairing_cancelled');

    if (command is RenameContactCommandDto) {
      var found = false;
      final name = command.displayName.trim();
      if (name.isEmpty || name.length > 64) return const BridgeResultDto(ok: false, kind: 'error', error: 'invalid contact name');
      final contacts = current.contacts.map((contact) {
    ...
    if (command is BlockContactCommandDto || command is UnblockContactCommandDto) {
      final id = command is BlockContactCommandDto ? command.contactIdHex : (command as UnblockContactCommandDto).contactIdHex;
      ...
      _snapshots.value = _copy(current, contacts: contacts);
      return BridgeResultDto(ok: true, kind: next == 'blocked' ? 'contact_blocked' : 'contact_unblocked');
    }
    if (command is RemoveContactCommandDto) {
      final conversations = current.conversations.where((item) => item.contactId == command.contactIdHex).map((item) => item.id).toSet();
      if (!current.contacts.any((item) => item.id == command.contactIdHex)) return const BridgeResultDto(ok: false, kind: 'error', error: 'contact not found');
      final messagesToRemove = current.messages.where((item) => conversations.contains(item.conversationId)).map((item) => item.id).toSet();
      _snapshots.value = _copy(
        current,
        contacts: current.contacts.where((item) => item.id != command.contactIdHex).toList(growable: false),
        conversations: current.conversations.where((item) => item.contactId != command.contactIdHex).toList(growable: false),
        messages: current.messages.where((item) => !conversations.contains(item.conversationId)).toList(growable: false),
        attachments: current.attachments.where((item) => !messagesToRemove.contains(item.messageId)).toList(growable: false),
      );
      return const BridgeResultDto(ok: true, kind: 'contact_removed');
    }
    if (command is ClearConversationHistoryCommandDto) {
      final removed = current.messages.where((item) => item.conversationId == command.conversationIdHex).map((item) => item.id).toSet();
      _snapshots.value = _copy(
        current,
        messages: current.messages.where((item) => item.conversationId != command.conversationIdHex).toList(growable: false),
        attachments: current.attachments.where((item) => !removed.contains(item.messageId)).toList(growable: false),
      );
      return const BridgeResultDto(ok: true, kind: 'conversation_history_cleared');
    }

    if (command is QueueMessageCommandDto) {
      if (command.replyToMessageId != null && !current.messages.any((message) => message.id == command.replyToMessageId)) {
        return const BridgeResultDto(ok: false, kind: 'error', error: 'reply message not found');
      }
      final message = MessageDto(
        id: command.messageIdHex, conversationId: command.conversationIdHex, body: command.body,
        direction: 'outbound', status: 'queued', replyToMessageId: command.replyToMessageId,
      );
    ...
    if (command is RetryMessageCommandDto) {
      ...
      final messages = current.messages.map((message) {
        ...
        found = true;
        if (message.direction != 'outbound' || message.status != 'failed') { invalid = true; return message; }
        return MessageDto(
          id: message.id, conversationId: message.conversationId, body: message.body,
          direction: message.direction, status: 'queued', replyToMessageId: message.replyToMessageId,
        );
      }).toList(growable: false);
      if (!found || invalid) return const BridgeResultDto(ok: false, kind: 'error', error: 'message is not retryable');
    ...
    if (command is MarkConversationReadCommandDto) {
      final messages = current.messages.map((message) {
        if (message.conversationId != command.conversationIdHex || message.direction != 'inbound' || message.status != 'delivered') return message;
        return MessageDto(
          id: message.id, conversationId: message.conversationId, body: message.body,
          direction: message.direction, status: 'read', replyToMessageId: message.replyToMessageId,
        );
      }).toList(growable: false);
      _snapshots.value = _copy(current, messages: messages);
  ...
  @override
  Future<String> diagnosticsJson() async => '{"events":[{"component":"Engine","state":"Ready","code":"MEMORY_PREVIEW"}]}';

  BridgeResultDto _terminalPairing(AppSnapshotDto current, String id, String state, String kind) =>
      _updatePairing(current, id, (pairing) => PairingDto(
        id: pairing.id, code: pairing.code, role: pairing.role, state: state,
        expiresAtMs: pairing.expiresAtMs, localApproved: pairing.localApproved, remoteApproved: pairing.remoteApproved,
      ), kind);

  BridgeResultDto _updatePairing(AppSnapshotDto current, String id, PairingDto Function(PairingDto) update, String kind) {
    var found = false;
    ...
    if (!found) return const BridgeResultDto(ok: false, kind: 'error', error: 'pairing session not found');
