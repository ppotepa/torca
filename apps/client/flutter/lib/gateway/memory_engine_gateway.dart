  ...
  Future<BridgeResultDto> execute(BridgeCommandDto command) async {
    ...
    if (command is RenameContactCommandDto) {
      ...
      if (name.isEmpty || name.length > 64) {
        return const BridgeResultDto(ok: false, kind: 'error', error: 'invalid contact name');
      }
      ...
      if (!found) {
        return const BridgeResultDto(ok: false, kind: 'error', error: 'contact not found');
      }
    ...
    if (command is BlockContactCommandDto || command is UnblockContactCommandDto) {
      ...
      if (!found || !valid) {
        return const BridgeResultDto(ok: false, kind: 'error', error: 'invalid contact transition');
      }
      _snapshots.value = _copy(current, contacts: contacts);
