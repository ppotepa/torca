...
class _ContactDetailsScreenState extends State<ContactDetailsScreen> {
  ...
  String? _normalizedSafety(String? value) {
    ...
    if (normalized == null || normalized.isEmpty || !RegExp(r'^[0-9A-F]+$').hasMatch(normalized)) {
      return null;
    }
    return normalized;
  }
  ...
  Future<void> _scanAndVerify(ContactDto contact) async {
    ...
    if (_safetyFromUri(scanned) != expected) {
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(content: Text('Safety Number does not match this contact.')),
      );
      return;
    }
  ...
  String? _safetyFromUri(String value) {
    ...
    if (uri == null || uri.scheme != 'torca' || uri.host != 'verify' || uri.queryParameters['v'] != '1') {
      return null;
    }
  ...
  Future<void> _toggleBlock(ContactDto contact) async {
    ...
    if (mounted && !result.ok) {
      _showError(result, blocking ? 'Could not block contact' : 'Could not unblock contact');
    }
  }
