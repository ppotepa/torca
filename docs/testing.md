...
The build jobs currently provide a deterministic Tor rendezvous endpoint because Tor is the default provider used by that CI path. Provider contract/unit coverage and device soak are separate evidence.
...
The soak CLI exposes Tor and Iroh as communication-provider choices. Scenarios can use managed/external relay modes where applicable; Iroh does not require a managed Tor rendezvous service.
