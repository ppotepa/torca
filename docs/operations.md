...
`torca-deploy` is the canonical planner/executor for local build, deploy, run, relay maintenance, logs and resume operations.
...
Every deployment plan selects one communication provider. Tor is the default; Tor and Iroh are currently normal selectable providers.

Provider commissioning requirements are data in the shared provider profile:

- Tor requires the managed rendezvous endpoint/service.
- Iroh does not require the managed Tor rendezvous service.
- WebRTC remains hidden until required host bridges exist.
...
Client data reset and provider-service maintenance are deliberate operations. A reset may remove identity, encrypted history and provider cache/secret state. Provider service rotation can invalidate client assumptions/artifacts and is guarded by deploy planning.
