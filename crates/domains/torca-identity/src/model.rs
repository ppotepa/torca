...
impl ProfileName {
    ...
    pub fn new(value: impl Into<String>) -> Result<Self, ProfileError> {
        ...
        if length == 0 { return Err(ProfileError::EmptyName); }
        if length > Self::MAX_CHARS { return Err(ProfileError::NameTooLong { actual: length }); }
        if trimmed.chars().any(char::is_control) { return Err(ProfileError::ControlCharacter); }
...
impl AvatarReference {
    ...
    pub fn new(value: impl Into<String>) -> Result<Self, ProfileError> {
        let value = value.into();
        if value.is_empty() { return Err(ProfileError::EmptyAvatarReference); }
        if value.len() > Self::MAX_BYTES { return Err(ProfileError::AvatarReferenceTooLong { actual: value.len() }); }
        if value.chars().any(char::is_control) { return Err(ProfileError::ControlCharacter); }
        Ok(Self(value))
    }
...
impl Profile {
    ...
    pub const fn display_name(&self) -> &ProfileName { &self.display_name }
    /// Returns the optional avatar reference.
    pub const fn avatar(&self) -> Option<&AvatarReference> { self.avatar.as_ref() }
}
...
impl IdentityKey {
    ...
    pub fn new(key_id: KeyId, algorithm: KeyAlgorithm, public_key: Vec<u8>) -> Result<Self, ProfileError> {
        if public_key.is_empty() { return Err(ProfileError::EmptyPublicKey); }
        Ok(Self { key_id, algorithm, public_key })
    }
    /// Returns the key identifier.
    pub const fn key_id(&self) -> KeyId { self.key_id }
    /// Returns the algorithm identifier.
    pub const fn algorithm(&self) -> KeyAlgorithm { self.algorithm }
...
impl PublicIdentity {
    ...
    pub const fn key(&self) -> &IdentityKey { &self.key }
...
impl Identity {
    ...
    pub const fn public(&self) -> &PublicIdentity { &self.public }
    ...
    /// Updates the profile.
    pub fn update_profile(&mut self, profile: Profile, at: Timestamp) { self.profile = profile; self.updated_at = at; }
...
#[must_use]
#[derive(Clone, Debug, Eq, PartialEq)]
pub struct CreateIdentity { pub identity_id: IdentityId, pub profile: Profile, pub at: Timestamp }
/// Input for profile mutation.
#[must_use]
#[derive(Clone, Debug, Eq, PartialEq)]
pub struct UpdateProfile { pub profile: Profile, pub at: Timestamp }
/// Input for key rotation.
#[must_use]
#[derive(Clone, Copy, Debug, Eq, PartialEq)]
pub struct RotateIdentity { pub at: Timestamp }

/// Event emitted after identity creation.
#[must_use]
#[derive(Clone, Debug, Eq, PartialEq)]
pub struct IdentityCreated { pub public_identity: PublicIdentity, pub at: Timestamp }
/// Event emitted after profile mutation.
#[must_use]
#[derive(Clone, Debug, Eq, PartialEq)]
pub struct ProfileUpdated { pub identity_id: IdentityId, pub profile: Profile, pub at: Timestamp }
/// Event emitted after key rotation.
#[must_use]
#[derive(Clone, Debug, Eq, PartialEq)]
pub struct IdentityKeyRotated { pub public_identity: PublicIdentity, pub previous_key_id: KeyId, pub at: Timestamp }
