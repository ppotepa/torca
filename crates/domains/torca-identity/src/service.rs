...
{
    /// Creates a service.
    pub const fn new(repository: R, key_provider: K) -> Self { Self { repository, key_provider } }
    /// Consumes the service and returns its ports.
    pub fn into_parts(self) -> (R, K) { (self.repository, self.key_provider) }
    /// Loads the local identity.
    pub fn load(&self) -> Result<Option<Identity>, IdentityError> { self.repository.load().map_err(Into::into) }

    /// Creates the local identity exactly once.
    pub fn create(&mut self, command: CreateIdentity) -> Result<(Identity, IdentityCreated), IdentityError> {
        if self.repository.load()?.is_some() { return Err(IdentityError::AlreadyExists); }
    ...
    pub fn update_profile(&mut self, command: UpdateProfile) -> Result<(Identity, ProfileUpdated), IdentityError> {
        ...
        if !self.repository.replace(generation, &identity)? { return Err(IdentityError::Conflict); }
    ...
    pub fn rotate(&mut self, command: RotateIdentity) -> Result<(Identity, IdentityKeyRotated), IdentityError> {
        let mut identity = self.repository.load()?.ok_or(IdentityError::NotFound)?;
        ...
        if !self.repository.replace(expected_generation, &identity)? { return Err(IdentityError::Conflict); }
