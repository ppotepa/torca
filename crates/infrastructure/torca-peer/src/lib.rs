...
pub struct PeerSession<T, V> {
    ...
    pending: BTreeMap<OpaqueId, PendingEnvelope>,
}
impl<T: PeerTransport, V: HandshakeVerifier> PeerSession<T, V> {
    ...
    pub fn connect(&mut self, hello: HandshakeHello) -> Result<(), PeerSessionError> {
        ...
        self.local_challenge = Some((hello.session_id, hello.nonce));
        self.transport.send(PeerCodec::encode(&PeerMessage::Hello(hello))?)?;
        Ok(())
    }
    pub fn send_handshake_ack(&mut self, ack: HandshakeAck) -> Result<(), PeerSessionError> {
        ...
        self.transport.send(PeerCodec::encode(&PeerMessage::HelloAck(ack))?)?;
    ...
    pub fn receive(
        ...
        match &message {
            PeerMessage::Hello(hello) => {
                ...
                self.policy.validate_hello(hello, now, &self.verifier)?;
            ...
            PeerMessage::HelloAck(ack) => {
                ...
                self.policy.validate_ack(ack, session, nonce, &self.verifier)?;
            ...
            PeerMessage::Ping(value) => {
                self.transport.send(PeerCodec::encode(&PeerMessage::Pong(*value))?)?;
            }
            PeerMessage::Pong(_) => {}
            PeerMessage::Data { .. } => {
    ...
    pub fn send_data(
        ...
        self.transport.send(PeerCodec::encode(&PeerMessage::Data {
            envelope_id,
            message_kind,
            ciphertext: record.ciphertext.clone(),
        })?)?;
        self.pending.insert(envelope_id, record);
        Ok(())
    }
