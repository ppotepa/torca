...
impl TorPeerTransport {
    /// Creates a disconnected transport. Onion validation remains at the SOCKS connector boundary
    ...
    fn encode_frame(&mut self, payload: &[u8]) -> Result<Vec<u8>, PeerTransportError> {
        ...
        self.next_wire_id = self.next_wire_id.checked_add(1)
        ...
        peer_wire_codec().encode(metadata, payload)
...
impl PeerTransport for TorPeerTransport {
    fn connect(&mut self) -> Result<(), PeerTransportError> {
        ...
        self.decoder.reset();
        self.received.clear();
        let stream = self.connector.connect_onion(&self.onion_address, self.port)
    ...
    fn try_receive(&mut self) -> Result<Option<Vec<u8>>, PeerTransportError> {
        if let Some(payload) = self.received.pop_front() { return Ok(Some(payload)); }
        self.read_available()?;
...
fn peer_wire_codec() -> WireCodec {
    let family = ProtocolFamily::new(PEER_PROTOCOL_FAMILY).expect("peer protocol family is non-zero");
