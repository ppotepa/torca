...
impl fmt::Display for BlobStoreError {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        match self {
            Self::Io(_) => f.write_str("blob storage I/O failure"),
            Self::NotFound => f.write_str("blob not found"),
            Self::TooLarge { actual } => write!(f, "blob exceeds size limit: {actual}"),
            Self::Crypto(error) => write!(f, "cryptographic failure: {error}"),
            Self::InvalidStoredBlob => f.write_str("stored blob is malformed"),
        }
    }
}
...
impl From<CryptoError> for BlobStoreError {
...
impl FileBlobStore {
    ...
    fn temporary_path(&self, id: AttachmentId) -> PathBuf {
        self.root.join(format!("{}.tmp", id))
    }
}
impl BlobStore for FileBlobStore {
    fn put_atomic(&mut self, id: AttachmentId, bytes: &[u8]) -> Result<(), BlobStoreError> {
        if bytes.len() > MAX_STORED_BLOB_BYTES {
            return Err(BlobStoreError::TooLarge { actual: bytes.len() });
        }
        let temporary = self.temporary_path(id);
        let final_path = self.path(id);
        let result = (|| -> Result<(), BlobStoreError> {
    ...
    fn read(&self, id: AttachmentId) -> Result<Vec<u8>, BlobStoreError> {
        ...
        let length = usize::try_from(metadata.len())
            .map_err(|_| BlobStoreError::TooLarge { actual: usize::MAX })?;
        if length > MAX_STORED_BLOB_BYTES {
            return Err(BlobStoreError::TooLarge { actual: length });
        }
        let mut bytes = Vec::with_capacity(length);
        File::open(path)?.take(MAX_STORED_BLOB_BYTES as u64 + 1).read_to_end(&mut bytes)?;
        if bytes.len() > MAX_STORED_BLOB_BYTES {
            return Err(BlobStoreError::TooLarge { actual: bytes.len() });
        }
        Ok(bytes)
...
impl<C: CryptoProvider, B: BlobStore> EncryptedAttachmentStore<C, B> {
    pub const fn new(crypto: C, blobs: B) -> Self {
        Self { crypto, blobs }
    }
    pub fn store(
        &mut self,
        ...
        associated_data: &[u8],
        plaintext: &[u8],
    ) -> Result<Nonce, BlobStoreError> {
        if plaintext.len() > MAX_ATTACHMENT_BYTES as usize {
            return Err(BlobStoreError::TooLarge { actual: plaintext.len() });
        }
        let mut nonce_bytes = [0_u8; 24];
        self.crypto.fill_random(&mut nonce_bytes)?;
        let nonce = Nonce(nonce_bytes);
        let ciphertext = self.crypto.seal(key, nonce, associated_data, plaintext)?;
        let mut stored = Vec::with_capacity(24 + ciphertext.0.len());
        stored.extend_from_slice(&nonce.0);
        stored.extend_from_slice(&ciphertext.0);
        self.blobs.put_atomic(id, &stored)?;
        Ok(nonce)
    }
    pub fn load(
        &self,
        ...
        let stored = self.blobs.read(id)?;
        let (nonce, ciphertext) =
            stored.split_at_checked(24).ok_or(BlobStoreError::InvalidStoredBlob)?;
        let nonce = Nonce(nonce.try_into().map_err(|_| BlobStoreError::InvalidStoredBlob)?);
        self.crypto
