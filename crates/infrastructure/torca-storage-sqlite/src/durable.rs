...
impl DurableDeliveryStore for InMemoryDurableDeliveryStore {
    fn queue_outbound(&mut self, message: Message, command_id: CommandId, next_attempt_at: Timestamp) -> Result<(), DurableDeliveryError> {
        ...
        self.outbox.insert(message.id(), OutboxRecord { message, command_id, attempts: 0, next_attempt_at, claimed_at: None, state: OutboxState::Pending });
        Ok(())
    }
    ...
    fn dead_letter(&mut self, message_id: MessageId) -> Result<(), DurableDeliveryError> {
        let record = self.outbox.get_mut(&message_id).ok_or(DurableDeliveryError::NotFound)?;
        if !matches!(record.state, OutboxState::Claimed | OutboxState::Pending) { return Err(DurableDeliveryError::InvalidState); }
        record.claimed_at = None;
    ...
    fn recover_stale_claims(&mut self, claimed_before: Timestamp) -> Result<usize, DurableDeliveryError> {
        ...
        for record in self.outbox.values_mut() {
            if record.state == OutboxState::Claimed && record.claimed_at.is_some_and(|at| at <= claimed_before) {
                record.state = OutboxState::Pending;
                record.claimed_at = None;
                recovered += 1;
            }
