...
impl SqlCipherControlOutbox {
    ...
    fn transition(
        ...
        let exists = self.backend.connection()
...
impl ControlOutboxStore for SqlCipherControlOutbox {
    fn queue(
        ...
        self.backend.connection().execute(
            INSERT_SQL,
            params![job.as_slice(), contact.as_slice(), kind as i64, payload, next_attempt_at.to_unix_millis()],
        ).map_err(|error| {
    ...
    fn claim_due(&mut self, now: Timestamp, limit: usize) -> Result<Vec<ControlJob>, ControlDeliveryError> {
        ...
        let mut statement = self.backend.connection().prepare(CLAIM_SQL)
            .map_err(|_| ControlDeliveryError::Backend)?;
