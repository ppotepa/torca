...
const ATTACHMENTS_FOR_CONVERSATION_SQL: &str = include_str!("../sql/queries/attachments_for_conversation.sql");
...
impl SqlCipherRelationshipAdmin {
    pub fn open(path: impl AsRef<Path>, key: &DatabaseKey) -> Result<Self, RelationshipAdminError> {
        let backend = SqlCipherBackend::open(path, key).map_err(|_| RelationshipAdminError::Storage)?;
    ...
    pub fn contact_names(&self) -> Result<BTreeMap<ContactId, String>, RelationshipAdminError> {
        let mut statement = self.backend.connection().prepare(LIST_NAMES_SQL)
    ...
    pub fn rename_contact(
        ...
        let changed = self.backend.connection().execute(
            UPSERT_NAME_SQL,
            params![id.as_slice(), display_name, at.to_unix_millis()],
        ).map_err(|_| RelationshipAdminError::Storage)?;
        if changed == 0 { return Err(RelationshipAdminError::NotFound); }
        Ok(())
    }

    pub fn block_contact(&mut self, contact_id: ContactId, at: Timestamp) -> Result<(), RelationshipAdminError> {
        self.transition_status(contact_id, 0, 1, at)
    }

    pub fn unblock_contact(&mut self, contact_id: ContactId, at: Timestamp) -> Result<(), RelationshipAdminError> {
    ...
    pub fn clear_conversation_history(
        ...
        let contact: Option<Vec<u8>> = self.backend.connection().query_row(
            CONVERSATION_CONTACT_SQL,
            params![conversation.as_slice()],
            |row| row.get(0),
        ).optional().map_err(|_| RelationshipAdminError::Storage)?;
        ...
        let result = (|| {
            self.backend.connection().execute(
                DELETE_CONTROL_SQL,
                params![contact_id.as_bytes().as_slice()],
            ).map_err(|_| RelationshipAdminError::Storage)?;
            self.backend.connection().execute(
                DELETE_MESSAGES_SQL,
                params![conversation.as_slice()],
    ...
    pub fn remove_contact(
        ...
        let context = self.backend.connection().query_row(
            CONTACT_CONTEXT_SQL,
            params![contact.as_slice()],
            |row| Ok((row.get::<_, i64>(0)?, row.get::<_, Option<Vec<u8>>>(1)?, row.get::<_, Option<Vec<u8>>>(2)?)),
        ).optional().map_err(|_| RelationshipAdminError::Storage)?
            .ok_or(RelationshipAdminError::NotFound)?;
        let conversation_id = context.1
            .map(fixed16)
            .transpose()?
            .map(OpaqueId::from_bytes)
            .map(ConversationId::from_opaque);
        let secret_handle = context.2
            .map(fixed16)
            .transpose()?
            .map(OpaqueId::from_bytes);
        ...
        self.backend.begin().map_err(|_| RelationshipAdminError::Storage)?;
        let result = (|| {
            self.backend.connection().execute(DELETE_CONTROL_SQL, params![contact.as_slice()])
                .map_err(|_| RelationshipAdminError::Storage)?;
            if let Some(conversation_id) = conversation_id {
                let conversation = conversation_id.to_opaque().into_bytes();
                self.backend.connection().execute(DELETE_MESSAGES_SQL, params![conversation.as_slice()])
                    .map_err(|_| RelationshipAdminError::Storage)?;
            }
            let deleted = self.backend.connection().execute(DELETE_CONTACT_SQL, params![contact.as_slice()])
                .map_err(|_| RelationshipAdminError::Storage)?;
            if deleted != 1 { return Err(RelationshipAdminError::NotFound); }
            Ok::<(), RelationshipAdminError>(())
        })();
        finish_transaction(&mut self.backend, result)?;
    ...
    fn transition_status(
        ...
        let changed = self.backend.connection().execute(
            UPDATE_STATUS_SQL,
            params![id.as_slice(), next, at.to_unix_millis(), expected],
        ).map_err(|_| RelationshipAdminError::Storage)?;
    ...
    fn attachments_for_conversation(
        ...
        let conversation = conversation_id.to_opaque().into_bytes();
        let mut statement = self.backend.connection().prepare(ATTACHMENTS_FOR_CONVERSATION_SQL)
            .map_err(|_| RelationshipAdminError::Storage)?;
        let rows = statement.query_map(params![conversation.as_slice()], |row| row.get::<_, Vec<u8>>(0))
            .map_err(|_| RelationshipAdminError::Storage)?;
        rows.map(|row| {
...
fn finish_transaction(
    ...
    match result {
        ...
        Err(error) => { let _ = backend.rollback(); Err(error) }
    }
}
