...
use rusqlite::params;
use torca_delivery::{ApplicationPayload, ApplicationPayloadCodec, DeliveryReceiptKind, ReceiptPayload};
...
pub enum ReadStateError { Storage, InvalidStoredId, Protocol }
impl core::fmt::Display for ReadStateError {
    fn fmt(&self, formatter: &mut core::fmt::Formatter<'_>) -> core::fmt::Result { write!(formatter, "{self:?}") }
}
impl std::error::Error for ReadStateError {}

pub struct SqlCipherReadState { backend: SqlCipherBackend }
