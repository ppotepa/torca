...
impl EngineBridge {
    ...
    pub fn attach_runtime(&mut self, runtime: RuntimeHostHandle) {
    ...
    pub fn execute(&self, command: BridgeCommand) -> BridgeResult {
        let result: Result<&'static str, String> = match command {
            ...
            BridgeCommand::QueueAttachment {
                ...
                .and_then(|request| self.runtime()?.queue_attachment(request).map_err(string_error))
...
fn map_snapshot(
    ...
    BridgeSnapshot {
        ...
        contacts: snapshot
            ...
            .into_iter()
            .map(|contact| {
                let connection_state = network
                    .peers
                    .get(&contact.id())
                    .map_or_else(|| "disconnected".to_owned(), |state| format!("{state:?}").to_lowercase());
                let peer_health = network.peer_health.get(&contact.id()).map_or_else(
                    ...
                    |health| BridgePeerHealth {
                        ...
                        last_success_at_ms: health.last_success_at.map(|at| at.to_unix_millis()),
                        consecutive_failures: health.consecutive_failures,
                        reconnect_attempt: health.reconnect_attempt,
                    },
                );
                let connection_state = peer_health.state.clone();
                let safety_number = local_public
                    .as_ref()
                    .map_or_else(String::new, |local| safety_number(local, contact.remote_identity()));
