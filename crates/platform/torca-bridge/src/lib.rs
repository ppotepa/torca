...
use torca_messaging::{Message, MessageBody, MessageDirection, MessageId, MessageStatus, ReplyReference};
...
pub enum BridgeCommand {
    ...
    QueueMessage { message_id_hex: String, conversation_id_hex: String, body: String, reply_to_message_id_hex: Option<String>, at_ms: i64 },
    ...
    QueueAttachment { attachment_id_hex: String, message_id_hex: String, conversation_id_hex: String, source_path: String, name: String, media_type: String, size: u64 },
...
#[must_use]
#[derive(Clone, Debug, Eq, PartialEq)]
pub struct BridgeContact { pub id: String, pub display_name: String, pub onion_address: String, pub status: String, pub connection_state: String, pub safety_number: String, pub peer_health: BridgePeerHealth, pub verification_status: String, pub verified_at_ms: Option<i64> }
#[must_use]
#[derive(Clone, Debug, Eq, PartialEq)]
...
#[must_use]
#[derive(Clone, Debug, Eq, PartialEq)]
pub struct BridgeMessagePage { pub messages: Vec<BridgeMessage>, pub has_more: bool }
#[must_use]
#[derive(Clone, Debug, Eq, PartialEq)]
...
impl EngineBridge {
    ...
    pub fn execute(&self, command: BridgeCommand) -> BridgeResult {
        let result: Result<&'static str, String> = match command {
            ...
            BridgeCommand::CreatePairing { session_id_hex } => parse_pairing_id(&session_id_hex).and_then(|id| runtime_command(self.runtime()?.create_pairing(id))).map(|_| "pairing_started"),
            BridgeCommand::JoinPairing { session_id_hex, code } => parse_pairing_id(&session_id_hex).and_then(|id| PairingCode::new(code).map_err(string_error).map(|code| (id, code))).and_then(|(id, code)| runtime_command(self.runtime()?.join_pairing(id, code))).map(|_| "pairing_joined"),
            BridgeCommand::ApprovePairing { session_id_hex } => parse_pairing_id(&session_id_hex).and_then(|id| runtime_command(self.runtime()?.approve_pairing(id))).map(|_| "pairing_updated"),
            BridgeCommand::RejectPairing { session_id_hex } => parse_pairing_id(&session_id_hex).and_then(|id| runtime_command(self.runtime()?.reject_pairing(id))).map(|_| "pairing_rejected"),
            BridgeCommand::CancelPairing { session_id_hex } => parse_pairing_id(&session_id_hex).and_then(|id| runtime_command(self.runtime()?.cancel_pairing(id))).map(|_| "pairing_cancelled"),
            BridgeCommand::RenameContact { contact_id_hex, display_name } => parse_contact_id(&contact_id_hex).and_then(|id| runtime_command(self.runtime()?.rename_contact(id, display_name))).map(|_| "contact_renamed"),
            BridgeCommand::VerifyContact { contact_id_hex } => parse_contact_id(&contact_id_hex).and_then(|id| runtime_command(self.runtime()?.verify_contact(id))).map(|_| "contact_verified"),
            BridgeCommand::ResetContactVerification { contact_id_hex } => parse_contact_id(&contact_id_hex).and_then(|id| runtime_command(self.runtime()?.reset_contact_verification(id))).map(|_| "contact_verification_reset"),
            BridgeCommand::BlockContact { contact_id_hex } => parse_contact_id(&contact_id_hex).and_then(|id| runtime_command(self.runtime()?.block_contact(id))).map(|_| "contact_blocked"),
            BridgeCommand::UnblockContact { contact_id_hex } => parse_contact_id(&contact_id_hex).and_then(|id| runtime_command(self.runtime()?.unblock_contact(id))).map(|_| "contact_unblocked"),
            BridgeCommand::RemoveContact { contact_id_hex } => parse_contact_id(&contact_id_hex).and_then(|id| runtime_command(self.runtime()?.remove_contact(id))).map(|_| "contact_removed"),
            BridgeCommand::ClearConversationHistory { conversation_id_hex } => parse_conversation_id(&conversation_id_hex).and_then(|id| runtime_command(self.runtime()?.clear_conversation_history(id))).map(|_| "conversation_history_cleared"),
            ...
            BridgeCommand::MarkConversationRead { conversation_id_hex } => parse_id(&conversation_id_hex).and_then(|id| runtime_command(self.runtime()?.mark_conversation_read(id))).map(|_| "conversation_read"),
            BridgeCommand::MarkConversationReadWithPolicy { conversation_id_hex, send_receipt } => parse_id(&conversation_id_hex).and_then(|id| runtime_command(self.runtime()?.mark_conversation_read_with_policy(id, send_receipt))).map(|_| "conversation_read"),
            BridgeCommand::QueueAttachment { attachment_id_hex, message_id_hex, conversation_id_hex, source_path, name, media_type, size } => parse_attachment_request(attachment_id_hex, message_id_hex, conversation_id_hex, source_path, name, media_type, size).and_then(|request| runtime_command(self.runtime()?.queue_attachment(request))).map(|_| "attachment_queued"),
            BridgeCommand::RetryAttachment { attachment_id_hex } => parse_id(&attachment_id_hex).and_then(|id| runtime_command(self.runtime()?.retry_attachment(id))).map(|_| "attachment_retried"),
            BridgeCommand::CancelAttachment { attachment_id_hex } => parse_id(&attachment_id_hex).and_then(|id| runtime_command(self.runtime()?.cancel_attachment(id))).map(|_| "attachment_cancelled"),
    ...
    pub fn snapshot(&self) -> Result<BridgeSnapshot, EngineError> { self.snapshot_internal(false) }
    pub fn full_snapshot(&self) -> Result<BridgeSnapshot, EngineError> { self.snapshot_internal(true) }

    fn snapshot_internal(&self, include_messages: bool) -> Result<BridgeSnapshot, EngineError> {
        let app = if include_messages { self.engine.snapshot()? } else { self.engine.overview_snapshot()? };
        ...
        Ok(map_snapshot(app, network, attachments, include_messages))
    }
...
fn runtime_command<T>(result: Result<T, RuntimeDriverError>) -> Result<(), String> {
    match result { Ok(_) | Err(RuntimeDriverError::Pending) => Ok(()), Err(error) => Err(string_error(error)) }
}
...
fn map_snapshot(snapshot: ClientSnapshot, network: NetworkSnapshot, attachments: Vec<AttachmentView>, include_messages: bool) -> BridgeSnapshot {
    ...
    let summaries = if include_messages { summarize_messages(&snapshot.messages) } else { BTreeMap::new() };
    let messages = if include_messages { snapshot.messages.into_iter().map(bridge_message_from_domain).collect() } else { Vec::new() };
    BridgeSnapshot {
        ...
        conversations: snapshot.conversations.into_iter().map(|conversation| {
            ...
            BridgeConversation {
                ...
                last_message_body: summary.and_then(|value| value.2.as_ref().map(|message| message.body().as_str().to_owned())),
                last_message_direction: summary.and_then(|value| value.2.as_ref().map(|message| format!("{:?}", message.direction()).to_lowercase())),
                last_message_status: summary.and_then(|value| value.2.as_ref().map(|message| format!("{:?}", message.status()).to_lowercase())),
            }
        }).collect(),
        messages,
...
fn summarize_messages(messages: &[Message]) -> BTreeMap<ConversationId, (u32, i64, Option<Message>)> {
    let mut values = BTreeMap::new();
    for message in messages {
        let entry = values.entry(message.conversation_id()).or_insert((0_u32, 0_i64, None));
        if message.direction() == MessageDirection::Inbound && message.status() == MessageStatus::Delivered {
            entry.0 = entry.0.saturating_add(1);
        }
        let activity = message.updated_at().to_unix_millis().max(message.created_at().to_unix_millis());
