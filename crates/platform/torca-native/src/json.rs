...
pub(crate) fn bridge_snapshot_json(snapshot: &BridgeSnapshot) -> String {
    ...
    for (index, contact) in snapshot.contacts.iter().enumerate() {
        ...
        output.push_str("\",\"verificationStatus\":\""); push_json_string(&contact.verification_status, &mut output);
        output.push_str("\",\"verifiedAtMs\":");
        match contact.verified_at_ms { Some(value) => { let _ = write!(output, "{value}"); } None => output.push_str("null") }
        output.push_str(",\"peerHealth\":{\"state\":\""); push_json_string(&contact.peer_health.state, &mut output);
        output.push_str("\",\"quality\":\""); push_json_string(&contact.peer_health.quality, &mut output);
        output.push_str("\",\"rttMs\":");
        match contact.peer_health.rtt_ms { Some(value) => { let _ = write!(output, "{value}"); } None => output.push_str("null") }
        output.push_str(",\"lastSuccessAtMs\":");
        match contact.peer_health.last_success_at_ms { Some(value) => { let _ = write!(output, "{value}"); } None => output.push_str("null") }
        let _ = write!(output, ",\"consecutiveFailures\":{},\"reconnectAttempt\":{}}}}}", contact.peer_health.consecutive_failures, contact.peer_health.reconnect_attempt);
    }
