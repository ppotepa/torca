...
use torca_transport_api::PeerTransportFactory;

use crate::composition::NativeCompositionError;

/// Dependencies supplied by the native host to the provider-owned pairing
/// implementation. They deliberately carry no provider-specific route type.
...
/// its neutral result and never constructs provider-specific components.
...
/// endpoint: a direct provider must never inherit unrelated service configuration.
