...
use torca_attachments::{AttachmentId, AttachmentName, MediaType, MAX_ATTACHMENT_BYTES};
...
fn validate_chunk(chunk: &AttachmentChunkFrame) -> Result<(), AttachmentProtocolError> {
    ...
    let length = u64::try_from(chunk.bytes.len()).map_err(|_| AttachmentProtocolError::ChunkTooLarge)?;
...
impl<'a> Cursor<'a> {
    ...
    fn u16(&mut self) -> Result<u16, AttachmentProtocolError> {
        Ok(u16::from_be_bytes(self.take(2)?.try_into().map_err(|_| AttachmentProtocolError::Truncated)?))
    }
    fn u32(&mut self) -> Result<u32, AttachmentProtocolError> {
        Ok(u32::from_be_bytes(self.take(4)?.try_into().map_err(|_| AttachmentProtocolError::Truncated)?))
    }
    fn u64(&mut self) -> Result<u64, AttachmentProtocolError> {
        Ok(u64::from_be_bytes(self.take(8)?.try_into().map_err(|_| AttachmentProtocolError::Truncated)?))
    }
    fn id(&mut self) -> Result<OpaqueId, AttachmentProtocolError> {
        Ok(OpaqueId::from_bytes(self.take(16)?.try_into().map_err(|_| AttachmentProtocolError::Truncated)?))
    }
