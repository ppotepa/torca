...
/// Maximum encoded pairing payload accepted before encryption.
pub const MAX_PAIRING_PAYLOAD_LEN: usize = 48 * 1024;
/// Maximum public-key byte length carried by an identity offer.
pub const MAX_PUBLIC_KEY_LEN: usize = 512;
/// Maximum onion hostname length.
pub const MAX_ONION_ADDRESS_LEN: usize = 255;
/// Maximum approval proof length.
...
/// Public identity and direct-route material offered by one pairing participant.
///
/// This value is not safe to give to the relay in plaintext. Callers encode it and then encrypt
/// the resulting bytes using the rendezvous key agreement for the current slot/session.
#[must_use]
...
pub struct PairingOffer {
    ...
    /// Per-offer random value included in transcript binding.
    pub transcript_nonce: [u8; 32],
}

impl PairingOffer {
    /// Validates bounds that must hold before an offer is encoded or accepted.
...
/// Explicit approval bound to a previously computed pairing transcript.
#[must_use]
#[derive(Clone, Debug, Eq, PartialEq)]
pub struct PairingApproval {
    /// Digest of the canonical transcript chosen by the application crypto layer.
    pub transcript_digest: [u8; 32],
    /// Signature/proof generated by the approving installation identity.
    pub proof: Vec<u8>,
}
...
/// Final confirmation that both participants bound the same transcript.
#[must_use]
#[derive(Clone, Copy, Debug, Eq, PartialEq)]
...
/// Pairing payload bound to one pairing session identifier.
#[must_use]
#[derive(Clone, Debug, Eq, PartialEq)]
...
/// Canonical envelope transported inside rendezvous encryption.
#[must_use]
#[derive(Clone, Debug, Eq, PartialEq)]
...
impl PairingEnvelope {
    ...
    /// Rejects replay of a valid encrypted payload into another pairing session.
    ...
    /// Encodes the deterministic plaintext representation to encrypt for rendezvous transport.
    ...
    /// Decodes and validates one decrypted rendezvous payload.
    ...
    /// Returns canonical transcript material for the application crypto layer to hash/sign.
    ///
    /// Approval and completion messages must use a digest computed from both ordered offer
    /// envelopes. The protocol intentionally does not choose a hash implementation.
...
/// Pairing payload validation/codec failure.
#[derive(Clone, Debug, Eq, PartialEq)]
