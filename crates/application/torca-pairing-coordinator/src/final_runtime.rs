...
pub struct LocalPairingContext {
    ...
    /// Capability this installation grants to the remote peer for future direct handshakes.
...
/// Application owner connecting encrypted rendezvous events to the pairing domain state machine.
///
/// The coordinator remains the sole owner of rendezvous/X25519 transport state. This runtime owns
/// only canonical offer transcript material and durable-finalization orchestration.
    ...
    S: PairingPeerSecretStore,
{
        ...
        for envelope in envelopes {
            match &envelope.payload {
                ...
                PairingPayload::Completion(completion) => {
                    self.apply_completion(session_id, *completion, now)?;
    ...
    fn transcript_digest(
        ...
        self.approval
            .transcript_digest(&creator, &joiner)
            .map_err(Into::into)
    }
