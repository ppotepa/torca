    ...
    S: PairingPeerSecretStore,
{
    ...
    pub fn create_invitation_pending_route(
        ...
        self.create_invitation_pending_route_with_bootstrap(session_id, now, None)
    }

    /// Creates a session-service invitation with optional provider-owned QR
    /// bootstrap data. The descriptor helps establish the initial session;
    /// the encrypted offer still carries the durable contact route.
    pub fn create_invitation_pending_route_with_bootstrap(
        &mut self,
        session_id: PairingSessionId,
        now: Timestamp,
        ...
        Ok(PairingInvitation {
            session_id,
            uri: crate::encode_invite_uri_with_bootstrap(&code, Some(&ticket), bootstrap)?,
            code,
            expires_at,
            ticket,
        })
    ...
    pub fn join_invitation(
        ...
        let (_, expires_at) = self.coordinator.join(session_id, &code, ticket, None)?;
        let context = self.coordinator.context(session_id)?;
        let local_offer = self.local_offer(context, local)?;
    ...
    pub fn join_invitation_pending_route(
        ...
        self.join_invitation_pending_route_with_bootstrap(session_id, code, ticket, None)
    }

    pub fn join_invitation_pending_route_with_bootstrap(
        &mut self,
        session_id: PairingSessionId,
        code: PairingCode,
        ticket: Option<[u8; 16]>,
