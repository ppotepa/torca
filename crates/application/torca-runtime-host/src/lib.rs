...
enum RuntimeCommand {
    ...
    Diagnostics(Sender<String>), Wake, Shutdown(Sender<()>),
}
...
fn run_loop<P: PairingDriver, C: CommunicationDriver, T: TorDriver>(receiver: Receiver<RuntimeCommand>, engine: &EngineHandle, pairing: &mut P, communication: &mut C, tor: &mut T, diagnostics: &mut DiagnosticBuffer, sequence: &mut u128) {
    ...
    loop {
        ...
        if last_tor_state != Some(tor_state) {
            ...
            record(diagnostics, sequence, now, Component::Tor, map_health(tor_state), "TOR_STATE_CHANGED");
        }
        if let Ok(snapshot) = engine.snapshot() {
            ...
            for id in contacts {
                ...
                if last_peer_states.get(&id) != Some(&state) { record(diagnostics, sequence, now, Component::Peer, map_peer_health(state), "PEER_STATE_CHANGED"); }
            ...
            let result = body.and_then(|body| {
                ...
                match communication.prepare_attachment(&request_value, now) {
                    ...
                    Err(error) => {
                        ...
                        engine.dispatch(EngineCommand::BeginMessageSend { message_id, at: now }).map_err(|_| RuntimeDriverError::Engine)?;
                        engine.dispatch(EngineCommand::MarkMessageFailed { message_id, at: now, error_code: failure_code }).map_err(|_| RuntimeDriverError::Engine)?;
                        Err(error)
                    }
                }
            });
...
fn request<T>(sender: &Sender<RuntimeCommand>, make: impl FnOnce(Sender<Result<T, RuntimeDriverError>>) -> RuntimeCommand) -> Result<T, RuntimeDriverError> {
    let (tx, rx) = mpsc::channel(); sender.send(make(tx)).map_err(|_| RuntimeDriverError::Communication)?; rx.recv().map_err(|_| RuntimeDriverError::Communication)?
}
...
fn record(buffer: &mut DiagnosticBuffer, sequence: &mut u128, at: Timestamp, component: Component, state: HealthState, code: &str) {
    let event_id = OpaqueId::from_u128(*sequence); *sequence = sequence.saturating_add(1);
...
const fn map_health(state: HostTorState) -> HealthState {
    match state { HostTorState::Stopped => HealthState::Stopped, HostTorState::Starting => HealthState::Starting, HostTorState::Ready => HealthState::Ready, HostTorState::Degraded => HealthState::Degraded, HostTorState::Failed => HealthState::Failed }
}
const fn map_peer_health(state: PeerConnectionState) -> HealthState {
    match state {
        ...
        PeerConnectionState::Connecting | PeerConnectionState::Handshaking | PeerConnectionState::Reconnecting => HealthState::Starting,
    }
}
