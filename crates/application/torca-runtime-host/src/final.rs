...
pub trait TorDriver: Send + 'static { fn maintenance(&mut self, now: Timestamp) -> Result<(), RuntimeDriverError>; fn state(&self) -> HostTorState; fn onion_address(&self) -> Option<String>; fn shutdown(&mut self); }

enum RuntimeCommand {
    CreatePairing(PairingSessionId, Sender<Result<PairingInvitationView, RuntimeDriverError>>), JoinPairing(PairingSessionId, PairingCode, Sender<Result<(), RuntimeDriverError>>), ApprovePairing(PairingSessionId, Sender<Result<(), RuntimeDriverError>>), RejectPairing(PairingSessionId, Sender<Result<(), RuntimeDriverError>>), CancelPairing(PairingSessionId, Sender<Result<(), RuntimeDriverError>>),
...
impl RuntimeHostHandle {
    ...
    pub fn export_attachment(&self, id: AttachmentId, destination: PathBuf) -> Result<(), RuntimeDriverError> { request(&self.sender, |r| RuntimeCommand::ExportAttachment(id, destination, r)) }
    ...
    pub fn diagnostics_json(&self) -> Result<String, RuntimeDriverError> { let (tx, rx) = mpsc::channel(); self.sender.send(RuntimeCommand::Diagnostics(tx)).map_err(|_| RuntimeDriverError::Communication)?; rx.recv().map_err(|_| RuntimeDriverError::Communication) }
...
impl RuntimeHostOwner {
    pub fn spawn<P: PairingDriver, C: CommunicationDriver, T: TorDriver>(engine: EngineHandle, mut pairing: P, mut communication: C, mut tor: T) -> (RuntimeHostHandle, Self) {
        ...
        let join = thread::spawn(move || {
            ...
            let startup = current_timestamp().unwrap_or(Timestamp::UNIX_EPOCH); let _ = communication.recover(startup);
...
fn run_loop<P: PairingDriver, C: CommunicationDriver, T: TorDriver>(receiver: Receiver<RuntimeCommand>, engine: &EngineHandle, pairing: &mut P, communication: &mut C, tor: &mut T, diagnostics: &mut DiagnosticBuffer, sequence: &mut u128) {
    loop {
        ...
        let now = current_timestamp().unwrap_or(Timestamp::UNIX_EPOCH); let _ = tor.maintenance(now); let _ = pairing.maintenance(now);
        if let Ok(snapshot) = engine.snapshot() {
            let contacts: Vec<_> = snapshot.contacts.iter().filter(|contact| contact.status() == ContactStatus::Active).map(|contact| contact.id()).collect();
            let _ = communication.maintenance(&contacts, now);
            record(diagnostics, sequence, now, Component::Tor, map_health(tor.state()), "TOR_STATE"); record(diagnostics, sequence, now, Component::Peer, HealthState::Ready, "PEER_TICK"); record(diagnostics, sequence, now, Component::Storage, HealthState::Ready, "DELIVERY_TICK");
        }
    }
}
    ...
    match command {
        ...
        RuntimeCommand::QueueAttachment(request_value, r) => {
            ...
            let result = body.and_then(|body| {
                ...
                match communication.prepare_attachment(&request_value, now) { Ok(()) => Ok(()), Err(preparation_error) => { let failure_code = ErrorCode::new("ATTACHMENT_PREPARE").map_err(|_| RuntimeDriverError::Engine)?; engine.dispatch(EngineCommand::BeginMessageSend { message_id, at: now }).map_err(|_| RuntimeDriverError::Engine)?; engine.dispatch(EngineCommand::MarkMessageFailed { message_id, at: now, error_code: failure_code }).map_err(|_| RuntimeDriverError::Engine)?; Err(preparation_error) } }
            }); let _ = r.send(result);
        }
...
fn request<T>(sender: &Sender<RuntimeCommand>, make: impl FnOnce(Sender<Result<T, RuntimeDriverError>>) -> RuntimeCommand) -> Result<T, RuntimeDriverError> { let (tx, rx) = mpsc::channel(); sender.send(make(tx)).map_err(|_| RuntimeDriverError::Communication)?; rx.recv().map_err(|_| RuntimeDriverError::Communication)? }
