...
/// Empty authenticated peer-session envelope reserved for link health measurement.
...
pub fn classify_peer_health(
    ...
    if consecutive_failures >= 2
        || sample_age.is_some_and(|age| age > Duration::from_secs(90))
    {
...
impl CommunicationDriver for TorcaCommunicationDriver {
    ...
    fn maintenance(
        ...
        self.attachments
            ...
            .map_err(map_runtime)?;
        // Probe runs after durable work. Only one deterministic side initiates probes, so the
        // opposite peer remains free to acknowledge and simultaneous probe deadlocks are avoided.
...
mod tests {
    ...
    use super::{PeerHealthQuality, classify_peer_health};

    #[test]
    fn health_quality_uses_rtt_failures_and_freshness() {
        assert_eq!(
            classify_peer_health(Some(400), 0, Some(Duration::from_secs(1))),
            PeerHealthQuality::Excellent
        );
        assert_eq!(
            classify_peer_health(Some(800), 0, Some(Duration::from_secs(1))),
            PeerHealthQuality::Good
        );
        assert_eq!(
            classify_peer_health(Some(1500), 0, Some(Duration::from_secs(1))),
            PeerHealthQuality::Fair
        );
        assert_eq!(
            classify_peer_health(Some(300), 2, Some(Duration::from_secs(1))),
            PeerHealthQuality::Poor
        );
        assert_eq!(
            classify_peer_health(Some(300), 0, Some(Duration::from_secs(120))),
            PeerHealthQuality::Poor
        );
    }
}
