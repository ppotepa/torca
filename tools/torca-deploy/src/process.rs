...
impl CommandRunner for SystemCommandRunner {
    fn run(&self, command: &CommandSpec) -> Result<CommandOutput, ProcessError> {
        ...
        let out_thread = thread::spawn(move || read_stream(stdout, false));
        let err_thread = thread::spawn(move || read_stream(stderr, true));
...
fn read_stream<R: std::io::Read + Send + 'static>(reader: R, stderr: bool) -> String {
